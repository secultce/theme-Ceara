...
<div class="widget" ng-non-bindable>
	<h3><?php \MapasCulturais\i::_e("Links");?></h3>
        <?php if($this->isEditable()): ?>
            <a class="add js-open-editbox hltip"
		data-target="#editbox-links" href="#"
		data-dialog-callback="MapasCulturais.MetalistManager.updateDialog"
        ...
        <ul class="js-metalist widget-list js-slimScroll">
			...
			class="widget-list-item<?php if($this->isEditable()) echo ' is-editable'; ?>">
			<a class="js-metalist-item-display" title="Cadastrado em <?php echo $link->createTimestamp->format('d/m/Y á\s H:i:s')?>" href="<?php echo $link->value;?>"><span><?php echo $link->title;?></span></a>
                    <?php if($this->isEditable()): ?>
                        <div class="botoes">
				<a class="edit js-open-editbox hltip" data-target="#editbox-links"
					data-dialog-callback="MapasCulturais.MetalistManager.updateDialog"
					data-response-target="#link-<?php echo $link->id ?>"
					data-metalist-action="edit"
					data-item="<?php echo htmlentities(json_encode($link)) ?>" href="#"
					title='<?php \MapasCulturais\i::esc_attr_e('editar'); ?>'></a> <a
					class='delete js-metalist-item-delete hltip js-remove-item'
					data-href='<?php echo $link->deleteUrl ?>'
					data-target="#link-<?php echo $link->id ?>"
	...
	<div id="editbox-links" ng-non-bindable class="js-editbox mc-left"
		title="<?php \MapasCulturais\i::esc_attr_e('Editar Link'); ?>"
		...
		data-metalist-description-label="<?php \MapasCulturais\i::esc_attr_e('Descrição'); ?>">
            <?php if($this->controller->action == 'create'): ?>
                <span class="js-dialog-disabled"
			data-message="<?php \MapasCulturais\i::esc_attr_e('Para adicionar links você primeiro deve salvar.'); ?>"></span>
            <?php else: ?>
