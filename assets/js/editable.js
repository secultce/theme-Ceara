...
jQuery(function () {
   ...
   $.fn.editableform.buttons =
      '<button type="button" class="editable-cancel btn btn-default">cancelar</button> <button type="submit" class="editable-submit">ok</button>';
   ...
   var registration_valuer_form =
      "form.js--registration-valuers-include-exclude-form";
   ...
   $(".js-editable").on("shown", function (e, editable) {
      ...
      if ($(this).hasClass("js-mask-phone")) {
         ...
         var masks = MapasCulturais.phoneMasks
            ? MapasCulturais.phoneMasks
            : ["(00) 0000-00009", "(00) 00000-0000"];
         editable.input.$input.mask(masks[0], {
            onKeyPress: function (val, e, field, options) {
               if (masks.length > 1) {
                  for (var ii = 1; ii < masks.length; ii++) {
                     field.mask(
                        val.length > masks[ii - 1].length - 1
                           ? masks[ii]
                           : masks[ii - 1],
                        options
                     );
      ...
      if ($(this).hasClass("js-mask-cep")) {
         ...
         var masks = MapasCulturais.postalCodeMask
            ? [MapasCulturais.postalCodeMask]
            : ["00000-000"];
         editable.input.$input.mask(masks[0], {
            onKeyPress: function (val, e, field, options) {
      ...
      var placeholder = editable.input.$input.attr("placeholder"),
         possibleSize = placeholder
            ? Math.max(placeholder.length, editable.value.length + 5)
            : 0;
      if (possibleSize > 20) {
         ...
         editable.input.$input.on("keydown", function (e) {
            if (e.which == 9) {
               ...
               if (e.shiftKey) {
                  ...
                  $(this).blur();
                  $el.parents()
                     .prevAll(":has(.editable):first") // find the parent of the editable before this one in the markup
                     .find(".js-editable:last")
                     .editable("show"); // grab the editable and display it
               } else {
                  // just tab
                  $(this).blur();
                  $el.parents()
                     .nextAll(":has(.editable):first") // find the parent of the editable after this one in the markup
                     .find(".js-editable:first")
                     .editable("show"); // grab the editable and display it
               }
            }
         });
   ...
   $("input.human_crop").change(function () {
      if (!window.FileReader) return; // browser não suporta
      ...
      $("#editbox-human-crop")
         .attr("class", $formEditBox.attr("class"))
         .width(cropWidth)
         .height(cropHeight + 80);
      MapasCulturais.EditBox.open(
         "#editbox-human-crop",
         $($form.data("target"))
      );
   ...
   init: function () {
      $("body").on("click", ".banner-delete", function () {
         ...
         var result = window.confirm(
            MapasCulturais.gettext.editable["removeAgentBackground"]
         );
         $("#remove-background-button").toggleClass(
...
MapasCulturais.Editables = {
   ...
   init: function (editableEntitySelector) {
      this.baseTarget =
         MapasCulturais.baseURL + $(editableEntitySelector).data("entity");
      this.createAll();
   ...
   initSpacePublicEditable: function () {
      ...
      $("#editable-space-status").on("hidden", function (e, reason) {
         if ($(this).editable("getValue", true) == "1") {
            $("#editable-space-status").html(
               '<div class="venue-status"><div class="icon icon-publication-status-open"></div>' +
                  labels["freePublish"] +
                  '</div><p class="venue-status-definition">' +
                  labels["freePlublishDescription"] +
                  "</p>"
            );
         } else {
            $("#editable-space-status").html(
               '<div class="venue-status"><div class="icon icon-publication-status-locked"></div>' +
                  labels["restrictedPublish"] +
                  '</div><p class="venue-status-definition">' +
                  labels["restrictedPublishDescription"] +
                  "</p>"
            );
         }
      });
   ...
   getEditableElements: function () {
      if (MapasCulturais.isEditable)
         return $(
            ".js-editable, .js-editable-taxonomy, .js-editable-type, .js-editable-registrationCategory"
         );
      else return $(".js-xedit");
   },
      ...
      MapasCulturais.Editables.getEditableElements().each(function () {
         ...
         if (config.type === "date") {
            ...
            if (!$(this).data("timepicker")) {
               ...
               //datepicker with related timepicker field
               var $timepicker = $($datepicker.data("timepicker"));
               var $hidden = $(
                  '<input class="js-include-editable" type="hidden">'
               ).insertAfter($timepicker);

               $datepicker.attr(
                  "data-edit",
                  $datepicker.data("edit") + "_datepicker"
               );
               ...
               if ($timepicker.data("datetime-value"))
                  $hidden.editable(
                     "setValue",
                     moment($timepicker.data("datetime-value")).format(
                        "YYYY-MM-DD HH:mm"
                     )
                  );
               else $hidden.editable("setValue", "");
               ...
               $timepicker.on("save", function (e, params) {
                  ...
                  $hidden.editable(
                     "setValue",
                     moment($datepicker.editable("getValue", true)).format(
                        "YYYY-MM-DD"
                     ) +
                        " " +
                        params.newValue
                  ...
                  if (params.newValue) {
                     ...
                     $hidden.editable(
                        ...
                        moment(params.newValue).format("YYYY-MM-DD") +
                           " " +
                           $timepicker.editable("getValue", true)
                     );
                  } else {
   ...
   setButton: function (editableEntitySelector) {
      ...
      $submitButton.click(function () {
         ...
         var $editables = MapasCulturais.Editables.getEditableElements().add(
            ".js-include-editable"
         );
         ...
         if (action === "create") {
            ...
            target = MapasCulturais.createUrl(controller, "single", [
               $(editableEntitySelector).data("id"),
            ]);

            if (
               MapasCulturais.entity.status == 0 &&
               $button.data("status") == 1
            ) {
               var message =
                  MapasCulturais.request.controller === "event"
                     ? labels["confirmPublish"].replace(
                          "%s",
                          labels["this_" + MapasCulturais.request.controller]
                       )
                     : labels["confirmPublishFinal"].replace(
                          "%s",
                          labels["this_" + MapasCulturais.request.controller]
                       );
            ...
            ajaxOptions: {
               ...
               statusCode: {
                  202: function (response, statusText, r) {
                     var createdRequests = JSON.parse(
                           r.getResponseHeader("CreatedRequests")
                        ),
                     ...
                     if (
                        createdRequests &&
                        createdRequests.indexOf("ChildEntity") >= 0
                     ) {
                        ...
                        MapasCulturais.Messages.alert(
                           labels["requestChild"]
                              .replace("%s", typeName)
                              .replace("%s", "<strong>" + name + "</strong>")
                        );
                     }

                     if (
                        createdRequests &&
                        createdRequests.indexOf("EventProject") >= 0
                     ) {
                        name = $('[data-field-name="projectId"]').text();
                        $(".js-pending-project").show();
                        MapasCulturais.Messages.alert(
                           labels["requestEventProject"].replace(
                              "%s",
                              "<strong>" + name + "</strong>"
                           )
                        );
               ...
               if (response.error) {
                  ...
                  for (var p in response.data) {
                     ...
                     if (
                        ...
                        MapasCulturais.request.controller === "registration" &&
                        p === "owner"
                     ) {
                        firstShown = true; // don't show editable
                        $field = $("#agent_owner")
                           .parent()
                           .find(".registration-label span");
                     }
                     ...
                     for (var k in response.data[p]) {
                        if ($field.length) {
                           ...
                           var errorHtml =
                              '<span title="' +
                              response.data[p][k] +
                              '" class="danger hltip js-response-error" data-hltip-classes="hltip-danger"></span>';
                           try {
                              if (
                                 $field.parents("edit-box")["0"].localName ===
                                 "edit-box"
                              ) {
                  ...
                  if (
                     ...
                     if (response.url) {
                        document.location = MapasCulturais.createUrl(
                        ...
                        if (
                           MapasCulturais.request.controller !=
                              "registration" &&
                           (action === "create" ||
                              response.status != MapasCulturais.entity.status)
                        ) {
...
MapasCulturais.MetalistManager = {
      ...
      $(".js-metalist-form").ajaxForm({
         ...
         beforeSubmit: function (arr, $form, options) {
            ...
            if (group === "videos") {
               ...
               if (
                  parsedURL.attr("host").indexOf("youtube") === -1 &&
                  parsedURL.attr("host").indexOf("vimeo") === -1
               ) {
         ...
         success: function (response, statusText, xhr, $form) {
            ...
            switch (action) {
               case "edit":
                  ...
                  if (group === "videos") {
                     MapasCulturais.Video.getAndSetVideoData(
                        response.value,
                        $target.find(".js-metalist-item-display"),
                        MapasCulturais.Video.setupVideoGalleryItem
                     );
                  }
                  break;
   ...
   updateDialog: function ($caller) {
      ...
      if ($caller.data("metalist-action") === "edit") {
         ...
         $form.attr(
            "action",
            MapasCulturais.baseURL + "metalist/single/" + item.id
         );
      } else {
         if (group === "videos")
      ...
      $form
         .find("input.js-metalist-title")
         .attr("placeholder", $dialog.data("metalist-title-label"));
      $form
         .find("input.js-metalist-value")
         .attr("placeholder", $dialog.data("metalist-value-label"));
      ...
      if (group === "videos") {
         $form.find("input.js-metalist-value").on("change", function () {
            MapasCulturais.Video.getAndSetVideoData(
               $(this).val(),
               $form.find("input.js-metalist-title"),
               MapasCulturais.Video.setTitle
            );
         });
      }
   },
...
$(function () {
   function concatena_enderco() {
      ...
      if (cep && nome_logradouro && numero && bairro && municipio && estado) {
         var endereco = MapasCulturais.buildAddress(
            nome_logradouro,
            numero,
            complemento,
            bairro,
   ...
   $(
      "#En_Nome_Logradouro, #En_CEP, #En_Num, #En_Complemento, #En_Bairro, #En_Municipio,  #En_Estado"
   ...
   $("#En_CEP").on("hidden", function (e, params) {
      ...
      $.getJSON("/site/address_by_postalcode?postalcode=" + cep, function (r) {
         if (r.success) {
            $("#En_Nome_Logradouro").editable(
               "setValue",
               r.streetName != null ? r.streetName : ""
            );
            $("#En_Bairro").editable(
               "setValue",
               r.neighborhood != null ? r.neighborhood : ""
            );
            $("#En_Municipio").editable(
               "setValue",
               r.city != null ? r.city.nome : ""
            );
            $("#En_Estado").editable(
               "setValue",
               r.state != null ? r.state.sigla : ""
            );
            concatena_enderco();
         }
      });
